        ...
        }:
        {
          ...
          config = {
            ...
            listAllTargetFilesCommand = ''
              find "${config.source.file}" -type f -printf '%p\0'
            '';
            # allTargetFiles =
    ...
    in
    {
      ...
      config =
        let
          ...
          managedFilesList =
            let
              managedFilesTargetsCommands =
                l.attrsets.mapAttrsToList (_: file: file.listAllTargetFilesCommand) cfg.files
              ;
            in pkgs.runCommand "managed-files-list" {} (let x = ''
              set -eu
          ...
          updateFilesScript =
            ...
            pkgs.writeShellScript "update-managed-files" ''
              ...
              ${pkgs.jq}/bin/jq --raw-output -sS '([.[] | .managedFiles] | flatten | unique | { managedFiles: . }) + (.[0] | {_comment})' \
                > "${fileListPathNew}" \
                <(
                  cat "${builtins.trace "${managedFilesList}" managedFilesList}"
